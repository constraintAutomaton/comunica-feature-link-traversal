...
import type { FilterFunction, ISummary, SummaryCache, SummaryCacheEntry } from '@comunica/types-link-traversal';
...
export class ActorExtractLinksShapeIndex extends ActorExtractLinks {
  ...
  public static readonly SHAPE_INDEX_SUMMARY_METHOD_LABEL = 'shape index';
  ...
  private readonly cacheShapeIndex: boolean;
  ...
  public constructor(args: IActorExtractLinksShapeIndexArgs) {
    super(args);
    this.cacheShapeIndex = this.cacheShapeIndex ?? false;
  }
  ...
  public async run(action: IActionExtractLinks): Promise<IActorExtractLinksOutput> {
    ...
    if (this.cacheShapeIndex) {
      // TODO add a logger to report when a shape index is cached
      ActorExtractLinksShapeIndex.cacheShapeIndex(shapeIndex.value, action.context);
    }
  ...
  /**
   * Cache a shape index into the context.
   * Will not cache the index if the the key `KeyCacheSummaries.summaries` does not
   * exist in the context and return false.
   * @param {IShapeIndex} shapeIndex
   * @param {IActionContext} context
   * @returns Weither or not the shape index have been cached into in the context
   */
  public static cacheShapeIndex(shapeIndex: IShapeIndex, context: IActionContext): boolean {
    const cacheBySummaries: SummaryCache | undefined = context.get(KeyCacheSummaries.summaries);
    if (cacheBySummaries === undefined) {
      return false;
    }
