...
import {
  ActorExtractLinksRuleSet,
  DF,
  IRuleSet,
} from "../lib/ActorExtractLinksRuleSet";
import "@comunica/utils-jest";
import { error, result } from "result-interface";
import { ArrayIterator, AsyncIterator } from "asynciterator";
import { rdfParser } from "rdf-parse";
import Streamify from "streamify-string";
import { IActionExtractLinks } from "@comunica/bus-extract-links";
...
describe("ActorExtractLinksRuleSet", () => {
  ...
  describe("test", () => {
    ...
    it(`should not test given a context with no ${KeyReasoning.rules.name} key`, () => {
      ...
      return expect(actor.test(action)).resolves.toFailTest(
        `the key ${KeyReasoning.rules.name} is not defined in the context`
      );
    });
    ...
    it(`should test given a context with a${KeyReasoning.rules.name} key`, () => {
      ...
      return expect(actor.test(action)).resolves.toPassTestVoid();
    });
  });
  ...
  describe("run", () => {
    ...
    let action: IActionExtractLinks;
